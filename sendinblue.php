...
class ControllerExtensionModuleSendinBlue extends Controller {
	...
	public function index() {
		...
		// Check Version
		$this->CheckVersion();
		$data['Sendinblue_Module_Version'] = Sendinblue_Module_Version;
	...
	public function CheckVersion(){
		$ocs_sendinblue = $this->model_setting_setting->getSetting('ocs_sendinblue');
		$status = false;
		if(empty($ocs_sendinblue)){
			$this->load->model('localisation/country');
			$country_info = $this->model_localisation_country->getCountry($this->config->get('config_country_id'));
			$ocs_sendinblue_data = array(
				'ocs_sendinblue_version' => '1.0',
				'ocs_sendinblue_notify_status' =>  1,
				'ocs_sendinblue_date_added' =>  date('Y-m-d h:i:s'),
			);
			$this->model_setting_setting->editSetting('ocs_sendinblue', $ocs_sendinblue_data);
			$status=true;
		}elseif($ocs_sendinblue['ocs_sendinblue_version'] != Sendinblue_Module_Version){
			$ocs_sendinblue_data = array(
				'ocs_sendinblue_version' => Sendinblue_Module_Version,
				'ocs_sendinblue_notify_status' =>  1,
				'ocs_sendinblue_date_added' =>  date('Y-m-d h:i:s'),
			);
			$this->model_setting_setting->editSetting('ocs_sendinblue', $ocs_sendinblue_data);
		...
		if($status){
			...
			$message ="Store Email :".$this->config->get('config_email').'<br/>';
			$message .="Domain Name :".($this->config->get('config_url') ? $this->config->get('config_url') : HTTPS_CATALOG).'<br/>';
			$message .="Store Country :".(isset($country_info['name']) ? $country_info['name'] : '').'<br/>';
			$message .="Store Owner :".$this->config->get('config_owner').'<br/>';
			
			$subject = 'Sendinblue Extension installed';
	...
	public function create_default_templates() {
		...
		$files = glob(DIR_SYSTEM . "../sendinblue/templates/*.{html}", GLOB_BRACE);
	...
	public function ajax_import_customers() {
		...
		$filename = DIR_SYSTEM . '../sendinblue/csv/sib_import_customers.csv';
	...
	public function ajax_import_orders() {
		...
		$filename = DIR_SYSTEM . '../sendinblue/csv/sib_import_orders.csv';		
